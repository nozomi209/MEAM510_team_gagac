 ...
 * 目标：
 * - 面向电脑端操作（更简洁清晰）
 * - 保留键盘映射：↑↓←→ 控制；Z/X 原地转；C 攻击切换；Q/W 调速度；A/S 调转向；松开方向键自动 S
...
<body>
    ...
    <div class="grid">
      ...
      <div class="card">
        ...
        <div class="hint">
          ...
          - 原地转：<span class="kbd">Z</span> 左转，<span class="kbd">X</span> 右转<br />
          - 攻击切换：<span class="kbd">C</span> ON/OFF<br />
          - 调速度：<span class="kbd">Q</span> -5，<span class="kbd">W</span> +5（百分比）<br />
          - 调转向：<span class="kbd">A</span> -5，<span class="kbd">S</span> +5（百分比）<br />
          - 松开方向键/Z/X 自动 <span class="kbd">S</span>；也可按 <span class="kbd">Space</span> 急停
        </div>
      ...
      <div class="card">
        ...
        <details open>
          ...
          <details open>
            <summary>攻击伺服（SV1 / SV0）- 按 C 键切换</summary>
            <div class="row">
              <button class="btn" id="btnAttackToggle" style="background:#f7b5b5;min-width:180px;">Attack: OFF (按C切换)</button>
            </div>
          </details>
        </details>
      </div>
      ...
      <div class="card">
        <h2>参数面板</h2>
        <div class="hint" style="margin-bottom:10px;">参数调节已移至独立页面，点击下方按钮打开：</div>
        <a href="/param" class="btn" style="display:inline-block;background:#9fc5e8;text-decoration:none;text-align:center;">打开参数页面 →</a>
      </div>
    </div>

    <div class="footer">提示：连接 Servant AP 后打开本页；常见地址 <span class="kbd">192.168.4.1</span> | <a href="/param">参数页面</a></div>
  </div>
  ...
  <script>
    ...
    const btnAttackToggle = document.getElementById("btnAttackToggle");
    let attackOn = false;
    ...
    document.addEventListener("keyup", (e) => {
      const tag = (e.target && e.target.tagName) ? e.target.tagName.toUpperCase() : "";
      if (tag === "INPUT" || tag === "TEXTAREA") return;
      if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "z", "Z", "x", "X"].includes(e.key)) stopNow();
    });
    ...
    function toggleAttack() {
      ...
      if (attackOn) {
        sendCommand("SV1");
        btnAttackToggle.innerText = "Attack: ON (按C切换)";
        btnAttackToggle.style.background = "#ef4444";
      } else {
        sendCommand("SV0");
        btnAttackToggle.innerText = "Attack: OFF (按C切换)";
        btnAttackToggle.style.background = "#f7b5b5";
      }
    }
    if (btnAttackToggle) btnAttackToggle.onclick = toggleAttack;
